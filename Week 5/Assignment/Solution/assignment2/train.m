...
fprintf(1, 'Batchsize %d\n', batchsize);
fprintf(1, 'learning_rate %.5f\n', learning_rate);
fprintf(1, 'Momentum %.3f\n', momentum);
fprintf(1, 'Embedding layer %d Hidden Layer %d\n', numhid1, numhid2);
fprintf(1, 'Init_wt %.3f\n', init_wt);
...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    % BACK PROPAGATE.
    %% OUTPUT LAYER.
    hid_to_output_weights_gradient =  hidden_layer_state * error_deriv'; %(numhid2 X batchsize) x (batchsize x vocab_size)
    output_bias_gradient = sum(error_deriv, 2);
    back_propagated_deriv_1 = (hid_to_output_weights * error_deriv) ... %(numhid2 x vocab_size) x (vocab_size x batchsize)
    ...
    embed_to_hid_weights_gradient = embedding_layer_state * back_propagated_deriv_1'; % (numhid1*numwords x numhid2)
